...
sbt -Dbrowser="${BROWSER_TYPE:=$DEFAULT_BROWSER}" -Denvironment="${ENV:=local}" -Dsecurity.assessment=true -Dbrowser.usePreviousVersion=true "testOnly uk.gov.hmrc.test.ui.specs.* -- -n ZapTests"
