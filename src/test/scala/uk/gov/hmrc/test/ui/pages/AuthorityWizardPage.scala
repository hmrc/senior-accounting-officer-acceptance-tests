...
object AuthorityWizardPage extends CommonPage with SubmissionButtonSupport {
  ...
  def redirectToRegistration(config: AuthorityWizardConfig): Unit =
    redirectTo(config, url = RegistrationPage.stubEnrolmentUrl, assertUlr = RegistrationPage.pageUrl)

  def redirectToHomePage(config: AuthorityWizardConfig): Unit =
    redirectTo(config, url = redirectHomePageUrl, assertUlr = redirectHomePageUrl)

  private def redirectTo(config: AuthorityWizardConfig, url: String, assertUlr: String): Unit = {
    loadPage()
